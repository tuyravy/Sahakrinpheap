 <?php 
        ...
        $res=$this->DailyCmr_model->getdcmrsahakrinpheaceo($reportdate);
?><!-- page content -->
        ...
        <script src="<?php echo base_url();?>public/vendors/jquery/dist/jquery.min.js"></script>
            ...
            <div class="breadcrumb flat row nopadding" style="margin-top:-10px;">
                        ...
                        <a href="<?= site_url('daily/writtenoffcollection');?>" class="active">Loan Written-Off Collection</a>
                  ...
                  <div class="x_content">
                    <div class="demo-container" style="height:auto;">
                                ...
                                <form class="form-inline">
                                  <fieldset class="scheduler-border">
                                    <legend class="scheduler-border">Specific Period:</legend>
                                      ...
                                      <div class="form-group">
                                          <label>
                                              <input type="checkbox" value="0" name="keydetail" disabled> Detail Reports
                                         </label>
                                      </div>
                      ...
                      <div class="col-md-12 nopadding">
                          ...
                          <h2 id="in" style="text-align:left;">NPL AND WRITTEN-OFF REPORT_DAILY</h2>
                       ...
                       <table class="table table-bordered">
                          ...
                          <tr class="active">
                            ...
                            <th rowspan="3" style="padding:0px 0px 10px 0px;text-align:center">Branch</th>
                            <th colspan="9" style="padding:0px 0px 3px 0px;text-align:center">Balance vs Achievement</th> 
                            ...
                            <tr class="active">
                                ...
                                <th colspan="3" style="padding:0px 0px 0px 0px;text-align:center">Loan late>=90 Day</th>
                          ...
                          <tbody>
                                <?php 
                                    if(isset($_GET['datestart']))
                                    {
                                        if(isset($_GET['keydetail']))
                                        {
                                           ...
                                           $page = $this->uri->segment(2) ? $this->uri->segment(3):1;
                                            $WO=$this->DailyCmr_model->getWO($datestart,$dateend,$role,$brcode,$key,$sid,$page);
                                        }else
                                        {
                                            $datestart=date('Ymd',strtotime($_GET['datestart']));
                                            $dateend=date('Ymd',strtotime($_GET['dateend']));
                                    ...
                                    }else
                                    {
                                        $key=1;
                                        $page = $this->uri->segment(2) ? $this->uri->segment(3):1;
                                        $WO=$this->DailyCmr_model->getWO($reportdate,$reportdate,$role,$brcode,$key,$sid,$page);
                                    }
                                    $i=1;
                                ...
                                <tr style="text-align:right">
                                    ...
                                    <td><?= number_format($row->LoanWOCollect,0);$totalWOCollect+=$row->LoanWOCollect;?></td>
                                    <td><?php 
                                            
                                            if($row->LoanWOCollect==0 || $row->LoanWOBalance==0)
                                            {
                                                echo number_format(0,2);    
                                            }else
                                            {
                                               echo number_format($row->LoanWOCollect/$row->LoanWOBalance,2);
                                                $totalperWo+=$row->LoanWOCollect/$row->LoanWOBalance;
                                            } 
                                        
                                     ?></td>
                                    <td><?= number_format($row->Balance,0);?></td>
                                    <td><?= number_format($row->Collection,0);$totalCollection+=$row->Collection;?></td>
                                    <td><?php
                                            if($row->Collection==0 || $row->Balance==0)
                                            {
                                                echo number_format(0,2);
                                            }else
                                            {
                               ...
                               <?php }?>
                                    ...
                                    <td><?= number_format($totalperWo,2);?></td>
                                    <td style="text-align:center">Total:</td>
                                    <td><?= number_format($totalCollection,0);?></td>
                                    <td><?= number_format($totalperCollect,2);?></td>
                                    <td style="text-align:center">Total:</td>
                                    <td colspan="2" style="text-align:left"><?= number_format($totalAchievement,0);?></td>
                                    
                                    </tr>
                                   <?php 
                                            $grandtoal=$this->DailyCmr_model->getGrandTotalAll(1,$reportdate);
                                            foreach($grandtoal as $row){
                                    ?>
                                    <tr style="text-align:right" class="info">
                                    <td style="text-align:right" colspan="3">Grand Total:</td>                                    
                                    <td><?= number_format($row->Loan90Collect,0);?></td>
                                    <td><?= number_format($row->Persion,2);?></td>
                                    <td style="text-align:center">Total:</td>
                                    <td><?= number_format($row->LoanWOCollect,0);?></td>
                                    <td><?php if($row->LoanWOCollect==0 || $row->LoanWOBalance==0){echo 0;}else{ echo number_format($row->LoanWOCollect/$row->LoanWOBalance,2);}?></td>
                                    <td style="text-align:center">Total:</td>
                                    <td><?= number_format($row->Collection,0);?></td>
                                    <td><?php if($row->Collection==0||$row->Balance==0){ echo 0;}else{echo number_format($row->Collection/$row->Balance,2);}?></td>
                                    <td style="text-align:center">Total:</td>
                                    <td colspan="2" style="text-align:left"><?= number_format($row->AmtDefer,0);?></td>
