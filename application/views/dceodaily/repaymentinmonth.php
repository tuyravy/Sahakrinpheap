               ...
               <div class="dashboard_graph x_panel">
                <div class="container">
                  ...
                  <ul class="nav nav-tabs">
                    <li class="active"><a href="<?php echo site_url("daily/repaymentd");?>">
                        ...
                        <div class="col-md-12">                          
                          ...
                          <p id="in" style="text-align:left">Reports Date:
                              <?php
                                ...
                                {
                                    echo date('d-M-Y',strtotime($datereports));
                                }
                              ?>
                        ...
                        <table id="datatable-buttons5" class="table table-bordered  table-condensed f14">
                          ...
                          <tbody id="showrapayinmonth"> 
                               ...
                               <?php 
                                ...
                                $result=$this->DailyCmr_model->getRepaymentInMonthDCEO($page,$datereports);
                                $Principle=0;
                                $Interest=0;
                                $Penalty=0;
                                $AdminFee=0;
                              ...
                              <td style="text-align:right"><?php echo number_format($row->Principle+$row->Interest+$row->Penalty+$row->AdminFee+$row->Principle1+$row->Interest1+$row->Penalty1+$row->AdminFee1,0);$totalgranted+=$row->Principle+$row->Interest+$row->Penalty+$row->AdminFee+$row->Principle1+$row->Interest1+$row->Penalty1+$row->AdminFee1;?></td>
                          ...
                          <?php }?>
                            <tr style="text-align:right">
                              ...
                              <?php 
                                            $grandtoal=$this->DailyCmr_model->getGrandTotalAll(2,$datereports);
                                            foreach($grandtoal as $row){
                                ?>
                              <tr style="text-align:right" class="info">
                              <td style="text-align:center;white-space: nowrap;overflow: hidden;">Grand Total:</td>
                              ...
                              <td style="text-align:right"><?php echo number_format(round($row->Principle,-2),0);?></td>
                              <td style="text-align:right"><?php echo number_format(round($row->Interest,-2),0);?></td>    
                              <td style="text-align:right"><?php echo number_format(round($row->Penalty,-2),0);?></td> 
                              <td style="text-align:right"><?php echo number_format(round($row->AdminFee,-2),0);?></td> 
                              <td style="text-align:right"><?php echo number_format(round($row->Principle1+$row->Interest1+$row->Penalty1+$row->AdminFee1,-2),0);?></td>
                              <td style="text-align:right"><?php echo number_format(round($row->Principle1,-2),0);?></td>
                              <td style="text-align:right"><?php echo number_format(round($row->Interest1,-2),0);?></td>    
                              <td style="text-align:right"><?php echo number_format(round($row->Penalty1,-2),0);?></td> 
                              <td style="text-align:right"><?php echo number_format(round($row->AdminFee1,-2),0);?></td>
                            </tr>
                              <?php }?>
                          </tbody>
                        </table>
